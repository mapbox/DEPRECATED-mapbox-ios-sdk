...
Follow these steps in order to use the MapBox iOS SDK within your own Xcode 4 project using [CocoaPods](http://cocoapods.org).
 ...
 1. Obtain the TileJSON URL for your hosted map by going to the **Info** tab, clicking **Embed**, and copying the **TileJSON URL**. This URL contains all the metadata needed by the SDK in order to serve your map.
 ...
 1. At a minimum, `#import "RMMapView.h"` and instantiate an `RMMapView` in your code along with an `RMMapBoxSource` using `-initWithReferenceURL:` and an `NSURL` made from your **TileJSON URL** obtained above.
...
Follow these steps in order to use the MapBox iOS SDK within your own Xcode 4 project as a statically linked framework.
 ...
 1. Alias the assets in `MapBox.framework/Resources` into your project so that they get copied into the product.
 1. At a minimum, `#import <MapBox/MapBox.h>` and instantiate an `RMMapView` in your code along with an `RMMapBoxSource` using `-initWithReferenceURL:` and an `NSURL` made from your **TileJSON URL** obtained above.
...
Follow these steps in order to use the MapBox iOS SDK within your own Xcode 4 project while retaining the ability to edit the SDK source as you go.
 ...
 1. Add the following libraries to your app target's `Link Binary With Libraries` build phase:
   ...
   * `libsqlite3.dylib` (required for MBTiles and caching support)
   * `libz.dylib` (required for UTFGrid interactivity)
 1. Alias the assets in `MapView/Map/Resources` into your project so that they get copied into the product.
 1. At a minimum, `#import "RMMapView.h"` and instantiate an `RMMapView` in your code along with an `RMMapBoxSource` using `-initWithReferenceURL:` and an `NSURL` made from your **TileJSON URL** obtained above.
...
You must comply with the licensing terms of any map data in your application, including [MapBox Streets](/help/#provide_proper_attribution_when_required) if used. Currently, this involves `RMMapView`'s [viewControllerPresentingAttribution](/mapbox-ios-sdk/api/#//api/name/viewControllerPresentingAttribution) property. This will soon be improved and made automatic across all tile sources in use in the map view. 
