...
Carried over from the Route-Me engine is the ability to configure cache behavior with a property list file included as a resource in your project. Below is a sample `routeme.plist` file structure with inline comments. See the source to `RMTileCache.m` for more information. 
    ...
    <dict>
        ...
        <array>
            <dict>
                <key>capacity</key>
                <integer>10000</integer> (maximum tiles to store on disk)
                <key>expiryPeriod</key>
                <integer>86400</integer> (maximum seconds to store tiles on disk; overrides capacity; zero means unlimited)
                <key>minimalPurge</key>
                <integer>10</integer> (minimum number to purge at a time when clearing expired tiles)
                <key>strategy</key>
                <string>FIFO</string> (FIFO = first in, first out; other possibility is LRU = least recently used)
                <key>type</key>
                <string>db-cache</string> (indicate that we're configuring the database cache)
                <key>useCachesDirectory</key>
                <true/> (use the app Caches folder instead of Documents)
            </dict>
            <dict>
                <key>capacity</key>
                <integer>32</integer> (maximum number of tiles to store in memory)
                <key>type</key>
