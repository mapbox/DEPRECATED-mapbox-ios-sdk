...
2. In the Objective-C code, use the newly saved style's TileJSON URL and ensure that the map view's `adjustTilesForRetinaDisplay` property is set to `NO`. Tiles will then draw at the normal 256px size instead of being magnified to 512px for legibility.
